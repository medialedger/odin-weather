...
export async function getCurrent(loc) {
	try {
		const response = await fetch(`https://api.weatherapi.com/v1/current.json?key=2c2ea9ec44b64c4797120433232803&q=${loc}&aqi=no`);
		if (!response.ok) {
			throw new Error(`HTTP error: ${response.status}`);
		}
		const data = await response.json();
